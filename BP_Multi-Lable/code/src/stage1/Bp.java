...
public class Bp {  
    ...
    /** 
     * delta vector of the hidden layer�����ز��ƫ�У� . 
     */  
    private final double[] hidDelta;  
    /** 
     * output layer of the output layer��������ƫ�У�. 
     */  
   ...
   // private final double momentum;  
    ...
    /** 
     * weight matrix��Ȩ�أ� from input layer to hidden layer. 
     */  
    private final double[][] iptHidWeights;  
    /** 
     * weight matrix��Ȩ�أ� from hidden layer to output layer. 
     */  
    ...
    public Bp(int inputSize, int hiddenSize, int outputSize, double eta) {  
        ...
        randomizeWeights(iptHidPrevUptWeights);  
        randomizeWeights(hidOptPrevUptWeights);
    ...
    /** 
     * Constructor with default eta = 0.9. 
     *  
     * @param inputSize 
     * @param hiddenSize 
     * @param outputSize 
    ...
    public Bp(int inputSize, int hiddenSize, int outputSize) {  
        this(inputSize, hiddenSize, outputSize, 0.9);  
    }  
    ...
    public double[] test(double[] inData) {  
        ...
        System.arraycopy(inData, 0, input, 1, inData.length);  //�ӵ�һ�����Ƶ��ڶ���
        forward();  
    ...
    private void forward(double[] layer0, double[] layer1, double[][] weight) {  
        ...
        for (int j = 1, len = layer1.length; j != len; ++j) {  
            ...
            for (int i = 0, len2 = layer0.length; i != len2; ++i)  
                sum += weight[i][j] * layer0[i];  //��weight[o][j]��Ϊj��ƫ��
    ...
    private void adjustWeight(double[] delta, double[] layer,  
        ...
        for (int i = 1, len = delta.length; i != len; ++i) {  
            for (int j = 0, len2 = layer.length; j != len2; ++j) {  
                double newVal = eta * delta[i] * layer[j];  //eta��ѧϰ��
                weight[j][i] += newVal;  
